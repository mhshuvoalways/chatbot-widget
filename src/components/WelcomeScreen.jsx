...
const WelcomeScreen = ({ config, userPlan, onStartChat }) => {
  ...
  return (
    <div 
      ...
      style={{
        background: `linear-gradient(135deg, ${hexToRGBA(config.primaryColor, 0.2)} 0%, #ffffff 100%)`
      }}
    >
      <div className="mr-auto">
        {config.logoUrl ? (
          <img src={config.logoUrl} alt="Logo" className="w-[70px] h-[70px]" />
        ) : (
          <Bot className="w-[70px] h-[70px]" style={{ color: config.primaryColor }} />
        )}
      </div>
      
      <h1 
        className="m-0 mb-2 text-gray-800 text-[28px] font-bold w-4/5 mr-auto"
      >
